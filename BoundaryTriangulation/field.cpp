...
double Field::trySphereRoute(int src,int dest,bool dump /* = false */)
{
	assert(src >= 0 && src < globalField->nSensors
		&& dest >= 0 && dest < globalField->nSensors);

	// Deleted...

}
double Field::tryTreeRoute(int src, int dest, bool dump /* = false */)
{
	assert(src >= 0 && src < globalField->nSensors
		&& dest >= 0 && dest < globalField->nSensors);

	vector<Sensor*> fullPath;
	fullPath.reserve(200);
	int totalNumTrans = 0;
	Sensor* srcSensor = globalField->sensorPool + src;
	Sensor* destSensor = globalField->sensorPool + dest;
	srcSensor->treeRoutingToDest(destSensor,fullPath);

	vector<Sensor*> shortestPath;
	shortestPath.reserve(200);
	srcSensor->floodingRouteOnFullTopology(destSensor,shortestPath);

	/*system("del treeRoutRealandShortestPath /q");*/
	double stretch = (double)(fullPath.size() )/(double)(shortestPath.size());
	if (stretch > 1 && dump)
	{
		dumpShortestandRealPath(srcSensor,fullPath,shortestPath,"treeRoutRealandShortestPath");
	}
	fullPath.clear();
	shortestPath.clear();
	return stretch; 
}
void Field::dumpShortestandRealPath(Sensor* src,vector<Sensor*> & realPath,vector<Sensor*> & shortestPath,char* filename )
{
	assert(src);
	...
	if(err)
	{
		fprintf(stderr,"\nCan not open bvrRoute file to append!\n");
	}
	else
	{
		//write real path
		fprintf(fp,"~%d ",src->index);
		for (int i = 0; i< realPath.size(); ++i)
		{
			fprintf(fp,"%d ",realPath[i]->index);
		}
		fprintf(fp,"\n");
		//write shortest path
		fprintf(fp,"-%d ",src->index);
		...
		{
			fprintf(fp,"%d ",shortestPath[j]->index);
		}
...
void Field::PickingLandmarkWithSignificance(int khop)
{
	...
	for (int iType = STARTLEVEL; iType <= MAX_BOUNDARY_TYPE; ++ iType)
	{
...
void Field::findAChildForSensors()
{
	for (int i = 0; i < nSensors; ++ i)
	{
		//cerr<<"Sensor id: "<<i<<endl;
		Sensor* s = sensorPool + i;
		s->findAChild();
	}
}
...
void Field::buildItypeEdgeAttachedLandmarkNb(const int itype)
{
	for (int i = 0; i< iType2LandmarkSet[itype].size(); ++i)
	{
		Sensor* lm = iType2LandmarkSet[itype][i];
		lm->itypeEdgeAttachedLandmarkNb[itype].clear();
	}
	...
	for (int i = 0; i < iType2LandmarkSet[itype].size(); ++i)
	{
		...
		for (auto itEd = lm->iTypeAttachedEdges[itype].begin(); 
			      itEd!=lm->iTypeAttachedEdges[itype].end();
			   ++ itEd)
		{
			LineSegment* ed = *itEd;

			lm->itypeEdgeAttachedLandmarkNb[itype].insert(ed->start);
			lm->itypeEdgeAttachedLandmarkNb[itype].insert(ed->end);

		}

		lm->itypeEdgeAttachedLandmarkNb[itype].erase(lm);
	...
	for (int iType = STARTLEVEL; iType <= MAX_BOUNDARY_TYPE; ++ iType)
	{
		...
		for (int i = 0; i < nSensors; ++ i)
		{
			...
			if (s->iType2Landmark[iType] && s->iTypeLandmarkNeighbors[iType].size() == 1)
			{
				...
				// connect to the nearest itype landmark...
				int minDistLm = 0XFFFF;
				Sensor* minLM = NULL;
				for (auto itSlm = iType2LandmarkSet[iType].begin(); itSlm != iType2LandmarkSet[iType].end(); ++ itSlm)
				{
					Sensor* slm = *itSlm;
					if (slm != s && slm != *(s->iTypeLandmarkNeighbors[iType].begin()))
					{
						vector<Sensor*>minPath;
						slm->floodingOnItypeBoundaryRouteTo(iType,s,minPath);
						if (minPath.size() < minDistLm)
						{
							minDistLm = minPath.size();
							minLM = slm;
						}
					}
				}
				...
				if (minLM == NULL)
				{
					...
					cout<<s->index<<" only have 1 neighbor..."<<endl;
					cout<<"****************************************************"<<endl;
					cerr<<" can not mend it!!"<<endl;
					exit(EXIT_FAILURE);
				}

				s->iTypeLandmarkNeighbors[iType].insert(minLM);
				minLM->iTypeLandmarkNeighbors[iType].insert(s);
...
// 			itypeAttachEdgesToTriangles(iType);
		...
		int lineIndex = iTypeEdgesSet[iType].size() ;
		int triIndex  = iTypeTriangleSet[iType].size();
		...
		cerr<<"Connecting hole..."<<endl;		
		connectHole2SickLm(iType);
		cerr<<" Connect hole complete..."<<endl;

		set<Sensor*>bannedStartEndSensors;
		while (true)
		{
			vector<LineSegment*>sickEdges;
			
			// cerr<<"Filling holes..."<<endl;
			for (int iE = 0; iE < iTypeEdgesSet[iType].size(); ++iE)
			{
				if (iTypeEdgesSet[iType][iE].usedtimes < 2)
				{
...
// 				attachEdgesToTriangles();
				...
				if (holePath.size() == 3)
				{
					...
					if (postri >  -1)
					{
						...
						// there is a solitude triangle here.
						// connect two vertex to the nearest sick landmark.
						for (auto it = holePath.begin(); it != holePath.end(); ++ it)
						{
							Sensor* s = *it;
							bannedStartEndSensors.insert(s);
						}
		...
		//int unAttacchedLM = 0;
		set<Sensor*>unAttachedLandmarks;
		for_each(iType2LandmarkSet[iType].begin(),iType2LandmarkSet[iType].end(),[&](Sensor* s)
		{
			if (s->iTypeAttachedTriangles[iType].empty())
			{
				//++ unAttacchedLM;
				unAttachedLandmarks.insert(s);
				// cout<<"In level "<<iType<<" , "<<s->index<<" has no attachment."<<endl;
			}
		});

		// cout<<unAttacchedLM<<" landmarks in level "<<iType<<endl;
		// Remove the landmarks without attached edges.
		for (auto it_unA = unAttachedLandmarks.begin(); it_unA != unAttachedLandmarks.end(); ++ it_unA)
		{
			Sensor* unA = *it_unA;
			//update the landmark neighbors.
			iType2LandmarkSet[iType].erase(remove(iType2LandmarkSet[iType].begin(),iType2LandmarkSet[iType].end(),unA),
											      iType2LandmarkSet[iType].end());
		}
		for (auto it_unA = unAttachedLandmarks.begin(); it_unA != unAttachedLandmarks.end(); ++ it_unA)
		{
			...
			{
				Sensor* lm = iType2LandmarkSet[iType][i];
				lm->iTypeLandmarkNeighbors[iType].erase(unA);
			}
			for (int i = 0; i < nSensors; ++ i )
			{
				Sensor* s = &(sensorPool[i]);
				if (s->iType2LandmarkIndex[iType] == unA->index)
				{
					// find the nearest landmark that not in the unattached set.
					int minLen2Lm = 0x00FF;
					Sensor* newNearestLm = NULL;
					for (int i = 0; i< iType2LandmarkSet[iType].size(); ++ i)
					{
						vector<Sensor*>path2Lm;
						s->floodingOnItypeBoundaryRouteTo(iType,iType2LandmarkSet[iType][i],path2Lm);
						if (path2Lm.size() < minLen2Lm)
						{
							minLen2Lm = path2Lm.size();
							newNearestLm = iType2LandmarkSet[iType][i];
						}
					}
					
					s->iType2LandmarkIndex[iType] = newNearestLm->index;
		...
		for (auto i_Lm = iType2LandmarkSet[iType].begin(); i_Lm != iType2LandmarkSet[iType].end(); ++ i_Lm)
		{
			Sensor* ilm = *i_Lm;

			if (!ilm->iType2Landmark[iType])
			{
				cerr<<ilm->index<<endl;
				cerr<<"Error, the sensor should be a landmark."<<endl;
				cerr<<"Error in "<<__FILE__<<" at "<<__LINE__<<" in "<<__FUNCTION__<<endl;
				exit(EXIT_FAILURE);
			}

			ilm->iType2LandmarkIndex[iType] = ilm->index;
...
void Field::findItype3Hole(const int iType)
{
	int debugNUmber = 0;
	//set<int>findTris;
	vector<LineSegment*>sickEdges;
			...
			it_iSickEdge != sickEdges.end();++it_iSickEdge)
		{
			...
			if (holePath.size() == 3)
			{
				...
				if (postri >  -1)
				{
					// cerr<<"Here is not hole."<<endl;
					// there is a solitude triangle here.
					cerr<<"Insert hole here..."<<endl;
					iTypeHoleset[iType].insert(postri);
...
void Field::connectHole2SickLm(const int iType)
{
	int lineIndex = iTypeEdgesSet[iType].size();
	set<Sensor*>sickLm;
	for (int i=0; i<iType2LandmarkSet[iType].size(); ++i)
	...
	set<Sensor*>minLenSensors;
	set<Sensor*> sickLm_copy;
	sickLm_copy = sickLm;

	//cerr<<"Finding 3holes.."<<endl;
	findItype3Hole(iType);
	//cerr<<"3holes found"<<endl;
	if (iTypeHoleset[iType].empty())
	{
		//there is no hole.
		return;
	}
	...
	{
		// connect to the sick landmarks not contain in the hole.
		// sick landmarks in the hole.
		set<Sensor*> sickLmInHole;
		for (auto it_itri = iTypeHoleset[iType].begin(); it_itri != iTypeHoleset[iType].end(); ++ it_itri)
		{
			sickLmInHole.insert(iTypeTriangleSet[iType][*it_itri].t1);
			sickLmInHole.insert(iTypeTriangleSet[iType][*it_itri].t2);
			sickLmInHole.insert(iTypeTriangleSet[iType][*it_itri].t3);

		}
		for (auto it_sInHole = sickLmInHole.begin(); it_sInHole != sickLmInHole.end(); ++ it_sInHole)
		{
			// remove the attached edge sensors.
			Sensor* sInHole = *it_sInHole;

			for (auto it_sAttachedEdge = sInHole->iTypeAttachedEdges[iType].begin();
				      it_sAttachedEdge != sInHole->iTypeAttachedEdges[iType].end(); ++ it_sAttachedEdge)
			{
				LineSegment* lattached = *it_sAttachedEdge;
				sickLm_copy.erase(lattached->start);
				sickLm_copy.erase(lattached->end);
			}
		}
		...
		for (auto it_sInHole = sickLmInHole.begin(); it_sInHole != sickLmInHole.end(); ++ it_sInHole)
		{
			...
			if (s1->iTypeAttachedEdges[iType].size() == 2)
			{
				auto it_min_len = min_element(sickLm_copy.begin(),sickLm_copy.end(),[&](Sensor* sa,Sensor* sb)->bool
				{
					LineSegment s1_sa(-1,s1,sa);
...
void Field::iTypeRicciFlow()
{
	...
	for (int i = STARTLEVEL; i <= MAX_BOUNDARY_TYPE; ++i)
	{
		if (i== banLevel)
		{
			continue;
		...
		iTypeRemovedSurfaceNodes[iType] = (miniTypeSensor);
		// remove a node....
...
void Field::ItypeStereographicProjection()
{
	//Clear the broadcast state.
	for (int i=0; i < nSensors; ++ i)
	{
	...
	for (int i = STARTLEVEL; i <= MAX_BOUNDARY_TYPE; ++i)
	{
		...
		int iType = i;
		buildItypeEdgeAttachedLandmarkNb(iType);

		set<Sensor*>iTypeLandmarks;
		for (int ilm = 0;ilm < iType2LandmarkSet[iType].size(); ++ilm)
		{
			Sensor* itypelm = iType2LandmarkSet[iType][ilm];
			if (itypelm->isBelongToType(iType))
			{
				iTypeLandmarks.insert(itypelm);
			}
		}
		Sensor* iRemoved = iTypeRemovedSurfaceNodes[iType];
		iTypeLandmarks.erase(iRemoved);

		Sensor* maxLenSensor = NULL;
		int maxLen = 0;
		int flen = 0;
		Sensor* v3 = *(iRemoved->itypeEdgeAttachedLandmarkNb[iType].begin());

		for_each(iTypeLandmarks.begin(),iTypeLandmarks.end(),[&](Sensor* sLm)
		{
			flen = v3->floodOnItypeEdgeAttachedLandmarkNbs(iType,sLm);
			if (flen > maxLen)
			{
				maxLenSensor = sLm;
			}
		...
		assert(maxLenSensor != v3 && maxLenSensor != iRemoved);
		vector<Sensor*> longestPath;
		v3->floodOnItypeEdgeAttachedLandmarkNbsRoutes(iType,maxLenSensor,longestPath);
		// v1 longestPath.back, v2 longestPath.middle, v3 iRemoved.
		assert(longestPath.size() > 2);
		// perform conformal mapping
		Sensor* v1 = longestPath.back();
		...
		complex<long double>z1(v1->iTypeRicciLocation[iType].x,v1->iTypeRicciLocation[iType].y);
		complex<long double>z2(v2->iTypeRicciLocation[iType].x,v2->iTypeRicciLocation[iType].y);
		complex<long double>z3(v3->iTypeRicciLocation[iType].x,v3->iTypeRicciLocation[iType].y);
		...
		for (auto itS = iTypeLandmarks.begin(); itS != iTypeLandmarks.end(); ++ itS)
		{
			...
			{
				complex<long double>mappedLocation;
				complex<long double>originalLocation(s->iTypeRicciLocation[iType].x,s->iTypeRicciLocation[iType].y);
				mappedLocation = (originalLocation - z1) * (z2 - z3)/((originalLocation - z3)*(z2 - z1));
				s->iTypeConformalLocation[iType].x = mappedLocation.real();
				s->iTypeConformalLocation[iType].y = mappedLocation.imag();
			}

		}
		// For v1, v2 and v3
		long double nan = std::numeric_limits<double>::quiet_NaN();
		v1->iTypeConformalLocation[iType] = Point(0,0);
		v2->iTypeConformalLocation[iType] = Point(1,0);
		v3->iTypeConformalLocation[iType] = Point(nan,nan);
		// sphere location
		for (auto itS = iTypeLandmarks.begin(); itS != iTypeLandmarks.end(); ++ itS)
		{
			Sensor* s = *itS;
			if ( s!= v3)
			{
				s->iTypeConforma2Sphere(iType);
			}

		}
		v3->iTypeSphereLocation[iType] = Point(0,0,1);
		
		Point center(0,0,0);
		for (auto it_IremovedNb = iRemoved->itypeEdgeAttachedLandmarkNb[iType].begin();
			it_IremovedNb != iRemoved->itypeEdgeAttachedLandmarkNb[iType].end();++ it_IremovedNb)
		{

			Sensor* IremovedNb = * it_IremovedNb;
			center.x += IremovedNb->iTypeSphereLocation[iType].x;
			center.y += IremovedNb->iTypeSphereLocation[iType].y;
			center.z += IremovedNb->iTypeSphereLocation[iType].z;

		}

		center.x /= iRemoved->itypeEdgeAttachedLandmarkNb[iType].size();
		center.y /= iRemoved->itypeEdgeAttachedLandmarkNb[iType].size();
		center.z /= iRemoved->itypeEdgeAttachedLandmarkNb[iType].size();
		
		long double modLen = center.distance(0,0,0);

		center.x /= modLen;
		center.y /= modLen;
		center.z /= modLen;

		iRemoved->iTypeSphereLocation[iType] = center;// should be ...
	}
...
void Sensor::floodOnItypeEdgeAttachedLandmarkNbsRoutes(const int itype, Sensor* destSensor,vector<Sensor*>& hopPath)
{
	// destSensor should not be null.
	assert(destSensor);
	// This node should be in the same boundary with destSensor.
	assert(destSensor->isBelongToType(itype));
	...
	if (spMapOnItypeEdgeAttachedLandmarks[itype].find(destSensor->index) != spMapOnItypeEdgeAttachedLandmarks[itype].end())
	{
		Sensor* curNode = this;

		while (curNode != destSensor)
		{
			Sensor *curParent = &globalField->sensorPool[curNode->spMapOnItypeEdgeAttachedLandmarks[itype][destSensor->index].second];
	...
	while (destSensor->broadcastCovered != 1)
	{
		...
		for(i = 0; i < (int)(frontier->size()); i++)
		{
			...
			{
				for (auto it_sNb = s->itypeEdgeAttachedLandmarkNb[itype].begin(); it_sNb != s->itypeEdgeAttachedLandmarkNb[itype].end(); ++ it_sNb)
				{
					Sensor *n = *it_sNb;
					// this condition limited the flooding on the boundary.
					if (n->broadcastCovered == 0 && n->isBelongToType(itype))
					{
						n->spMapOnItypeEdgeAttachedLandmarks[itype].insert(DEST2SP::value_type(index, SPLEN_NEXTHOP(hopCount + 1, s->index)));
	...
	do
	{
		...
		Sensor *curParent = &globalField->sensorPool[curNode->spMapOnItypeEdgeAttachedLandmarks[itype][index].second];
		//also remember the reverse route
		curParent->spMapOnItypeEdgeAttachedLandmarks[itype].insert(DEST2SP::value_type(destSensor->index, SPLEN_NEXTHOP(hopPath.size(), curNode->index)));
		curNode = curParent;
	}
	while (curNode != this);
...
int Sensor::floodOnItypeEdgeAttachedLandmarkNbs(const int itype, Sensor* destSensor)
{
	// destSensor should not be null.
	assert(destSensor);
	// This node should be in the same boundary with destSensor.
	...
	//the path is in the cache
	if (spMapOnItypeEdgeAttachedLandmarks[itype].find(destSensor->index) 
		!= spMapOnItypeEdgeAttachedLandmarks[itype].end())
	{
		return spMapOnItypeEdgeAttachedLandmarks[itype][destSensor->index].first;
	}
	...
	while (destSensor->broadcastCovered != 1)
	{
		...
		for(i = 0; i < (int)(frontier->size()); i++)
		{
			...
			if (s->broadcastCovered == 1)
			{
				for (auto it_sNb = s->itypeEdgeAttachedLandmarkNb[itype].begin();
					it_sNb != s->itypeEdgeAttachedLandmarkNb[itype].end(); ++ it_sNb)
				{
					Sensor *n = *it_sNb;
					// this condition limited the flooding on the boundary.
					if (n->broadcastCovered == 0 && n->isBelongToType(itype))
	...
	do
	{
		...
		Sensor *curParent = &globalField->sensorPool[curNode->spMapOnItypeEdgeAttachedLandmarks[itype][index].second];
		//also remember the reverse route
		curParent->spMapOnItypeEdgeAttachedLandmarks[itype].insert(DEST2SP::value_type(destSensor->index, SPLEN_NEXTHOP(pathLength, curNode->index)));
		curNode = curParent;
	}
	while (curNode != this);
...
void Sensor::iTypeConforma2Sphere(const int itype)
{
	long double denom =  1.0 + 
		pow(iTypeConformalLocation[itype].x,(long double)2.0) + pow(iTypeConformalLocation[itype].y,(long double)2.0);
	long double znume = -1.0 + 
		pow(iTypeConformalLocation[itype].x,(long double)2.0) + pow(iTypeConformalLocation[itype].y,(long double)2.0);

	iTypeSphereLocation[itype].x = (2 * iTypeConformalLocation[itype].x) / denom;
	iTypeSphereLocation[itype].y = (2 * iTypeConformalLocation[itype].y) / denom;
	iTypeSphereLocation[itype].z = znume / denom;
}
...
void Field::dumpItypeSphereLocation(string file_out_name)
{
	ofstream file_out;
	file_out.open(file_out_name.c_str(),ofstream::trunc);
	if ( !file_out )
	...
	{
		Sensor* s = &sensorPool[i];
		file_out << s->index<<endl;

		for (auto it_mapSphere = s->iTypeSphereLocation.begin(); it_mapSphere != s->iTypeSphereLocation.end(); ++ it_mapSphere)
		{

			file_out<<"~";
			file_out<<(*it_mapSphere).first<<" ";
			file_out<<(*it_mapSphere).second.x<<" ";
			file_out<<(*it_mapSphere).second.y<<" ";
			file_out<<(*it_mapSphere).second.z<<" ";
			file_out<<endl;
...
void Field::getUpLowLevelNbLandmarks()
{
	// for every landmark, get a up level neighbor and a low level neighbor.
	for (int i = STARTLEVEL; i < MAX_BOUNDARY_TYPE; ++i)
	{
		for (auto it_s  = iType2LandmarkSet[i].begin(); it_s != iType2LandmarkSet[i].end();++ it_s)
		{
			Sensor* s = * it_s;

			for (auto it_s1 = iType2LandmarkSet[i+1].begin(); it_s1 != iType2LandmarkSet[i+1].end(); ++ it_s1)
			{
				Sensor* s1 = *it_s1;
				// Todo ...
				if (s->inCommonType(s1) == -1)
				{
...
void Field::sphereAlignment()
{
	// 
	for (int i = STARTLEVEL; i< MAX_BOUNDARY_TYPE/* intended not put an equal here*/; ++ i)
	{
		//auto formerLevellm  = iType2LandmarkSet[i];
		//auto currentLevellm = iType2LandmarkSet[i+1];

// 		for (auto it_cur = currentLevellm.begin(); it_cur!= currentLevellm.end(); ++ it_cur)
// 		{
// 			Sensor* cur_s = *it_cur;
			// find the min distance landmark to the upper level.
// 			auto it_form_s = min_element(formerLevellm.begin(),formerLevellm.end(),[&](Sensor* s1,Sensor* s2)->bool
// 			{
// 				return(s1->floodingRouteOnFullTopologyLength(cur_s) < s2->floodingRouteOnFullTopologyLength(cur_s));
// 			});
	...
	for (int i = STARTLEVEL; i<= MAX_BOUNDARY_TYPE; ++ i)
	{
// 		vector<Sensor*>levelLM;
// 		levelLM  = iType2LandmarkSet[i];
		long double scaleFactor = (CORE_RADIUS + i) /** avgHopdistance*/;
		for (auto it_s  = iType2LandmarkSet[i].begin(); it_s != iType2LandmarkSet[i].end();++ it_s)
		{
			Sensor* s = * it_s;
			s->iTypeSphereLocation[i].x *=  scaleFactor;
			s->iTypeSphereLocation[i].y *=  scaleFactor;
			s->iTypeSphereLocation[i].z *=  scaleFactor;
		}
...
void Field::dumpItypeRicciLocation(string file_out_name)
{
	ofstream file_out;
	file_out.open(file_out_name.c_str(),ofstream::trunc);
	if ( !file_out )
	{
 	...
 	for (int i = 0; i < nSensors; ++ i)
 	{
		...
		for (auto it_mapRicci = s->iTypeRicciLocation.begin(); it_mapRicci != s->iTypeRicciLocation.end(); ++ it_mapRicci)
		{

			file_out<<"~";
			file_out<<(*it_mapRicci).first<<" ";
			file_out<<(*it_mapRicci).second.x<<" ";
			file_out<<(*it_mapRicci).second.y<<" ";
			file_out<<endl;
